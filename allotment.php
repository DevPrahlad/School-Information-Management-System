...
$host = "localhost";
$username = "ourwebpr_dk";
$password = "1u0lATw[*8dx";
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($getRegionNameStmt->fetch()) {
        ...
        foreach ($selectedSchools as $school) {
            // Insert the form data into the MySQL table, including the user ID and current school
            $sql = "INSERT INTO faculty_data (user_id, department, name, region_id, schools, target, tsdate, tedate, status)
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)";
            $stmt = $con->prepare($sql);
            $stmt->bind_param("issisisss", $loggedInUserId, $department, $name, $regionId, $school, $target, $tsdate, $tedate, $status);
