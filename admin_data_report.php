...
<head>
    ...
    .search-bar button:hover {
        border-color: #ff3333; /* Darker red on focus/hover */
        background-color: #ff3333; /* Darker red on focus/hover */
    }
...
<body>
    ...
    <section class="home-section">
        <nav>
            <div class="sidebar-button">
                ...
                <span class="dashboard">Completed Allotments</span>
            </div>
    ...
    if (isset($_GET['search'])) {
        $search = $_GET['search'];
        $sql = "SELECT id, name, schools, department, photo_path, creation_date FROM faculty_fill_data WHERE name LIKE '%$search%' ORDER BY creation_date DESC LIMIT $start_from, $records_per_page";
    } else {
        $sql = "SELECT id, name, schools, department, photo_path, creation_date FROM faculty_fill_data ORDER BY creation_date DESC LIMIT $start_from, $records_per_page";
    }
    ...
    if ($result->num_rows > 0) {
        echo "<table>";
        echo "<tr><th>Sr.No.</th><th>Person Name</th><th>Department</th><th>School</th><th>Image</th><th>Action</th></tr>";
        $cnt = 1;
...
echo "</td>";
            ...
            echo "<td><a href='user_report.php?id=$rowId' class='btn' onclick='removeNewMessage($rowId)'>View</a></td>";
