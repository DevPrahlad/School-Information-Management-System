...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $insert_query = "INSERT INTO schools (school_name, region_id) VALUES (?, ?)";
    $stmt = $con->prepare($insert_query);
    $stmt->bind_param("si", $schoolName, $regionId);
...
.dropdown:hover .dropdown-menu {
    display: block;
        ...
        .form-control {
            border: 1px solid #FF6961; /* Light red border */
            color: #333;
        }

        .form-control:focus {
            border-color: #ff3333; /* Darker red on focus */
            box-shadow: 0 0 0 0.2rem rgba(255, 102, 102, 0.25); /* Light red shadow on focus */
        }

        .btn-primary {
            background-color: #FF6961; /* Light red background */
            border-color: #FF6961; /* Light red border */
        }

        .btn-primary:hover {
            background-color: #FF4842; /* Darker shade of red on hover */
            border-color: #FF4842; /* Darker shade of red on hover */
        }
